...
static DEVICE_ATTR(gesture, S_IRUGO|S_IWUSR,DrvMainProcfsGestureWakeupModeRead ,DrvMainProcfsGestureWakeupModeWrite);
...
*/
	nRetVal = sysfs_create_group(&pClient->dev.kobj, &msg22xx_attribute_group);
    if (nRetVal != 0)
        DBG("*** sysfs_create_file() failed, nRetVal = %d ***\n", nRetVal);
