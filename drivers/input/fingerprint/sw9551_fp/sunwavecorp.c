...
static struct attribute *sunwave_debug_attrs[] = {
    &dev_attr_debug.attr,
    NULL
};

static const struct attribute_group sunwave_debug_attr_group = {
    .attrs = sunwave_debug_attrs,
    .name = "sunwave-debug"
};
...
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
    ...
    sunwave_dev->spi = spi;
	sunwave_dev->users = 0x5A;
    /** add register finger function && init device
     */
    ...
    if (status != 0) {
        ...
        int attr;
        attr = sysfs_create_group(&spi->dev.kobj,&sunwave_debug_attr_group);
        if(attr){
