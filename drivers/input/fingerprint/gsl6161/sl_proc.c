...
static void sl_proc_cmd_write(char*cmd)
{
   ...
   val_cmd = trim_sub_cmd(cmd);	
     FP_DBUG("%s,0x%x,%s\n", __func__, addr, val_cmd);
   if (!val_cmd) {
       FP_DBUG(KERN_ERR "%s:trim sub cmd failed\n", __func__ );
       return;
   }
   val_cmd = paser_sub_cmd(val_cmd);
   if (!val_cmd) {
       FP_DBUG(KERN_ERR "%s:paser sub cmd failed\n", __func__ );
       return;
   }
   val32 = simple_strtol(val_cmd, NULL, 16);
     FP_DBUG("%s,0x%x,%s,0x%x\n", __func__, addr, val_cmd, val32);
   if (addr &(~0xFF)){
...
extern struct cdev spicdev;
static void sl_proc_cmd_uevent(char *cmd)
{
    char *event[] = {"silead_fp", NULL};
    if (fp_spidev){
       kobject_uevent_env(&fp_spidev->spi->dev.kobj, KOBJ_CHANGE, event); 
    }
//       kobject_uevent_env(&spicdev.kobj, KOBJ_CHANGE, event); 
...
int sl_proc_init(struct spidev_data *spidev)
{
    ...
    //FP_DBUG("[%s] sl_proc = %x \n",__func__,sl_proc);
    if ( NULL == sl_proc) {
        FP_DBUG("create_proc_entry %s failed\n", SL_FP_PROC_FILE);
        return -1;
    }
